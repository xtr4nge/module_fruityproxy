...
$folder_selected = $_POST["screenshot-foldername"];
if ($folder_selected != "ALL") $folderName = $folder_selected;

$pathLen = 0;

function prePadScreenshot($level) {
	$ss = "";
...
function myScanDirScreenshot($dir, $level, $rootLen) {
	global $pathLen;
	global $folderName;
	...
	if ($handle = opendir($dir)) {
	  ...
	  foreach($allFiles as $value)
	  {
		...
		if (is_dir($fileName)) {
		  echo prePadScreenshot($level) . $linkName . "<br>\n";
		  myScanDirScreenshot($fileName, $level + 1, strlen($fileName));
		} else {
		  //echo prePad($level) . "<a href=\"" . $linkName . "\" style=\"text-decoration:none;\">" . $displayName . "</a><br>\n";
		  //$show_link = "<a href='includes/FruityProxy-master/logs/Screenshot/$folderName/" . $linkName . "' style='text-decoration:n-one;'>" . $displayName . "</a><br>\n";
		  /*
		  $show_link = "<a href='includes/FruityProxy-master/logs/Screenshot/$folderName/" . $linkName . "' style='text-decoration:n-one;'>" . $displayName . "</a><br>\n";
		  $show_link = preg_replace('~//+~', '/', $show_link);
		  */
		  $img_path = "includes/FruityProxy-master/logs/Screenshot/$folderName/$linkName";
		  ...
		  echo prePadScreenshot($level) . $show_link;
		  //echo prePadScreenshot($level) . "<a href='includes/FruityProxy-master/logs/Screenshot$folderName/" . $linkName . "' style='text-decoration:n-one;'>" . $displayName . "</a><br>\n";
		}
	  }
	}
}
...
?>
<?
$pathImages = "includes/FruityProxy-master/logs/Screenshot/";
$list = glob($pathImages.'*');

if ($folder_selected != "" and $folder_selected != "ALL") $folderName = $folder_selected;

?>
<form action="?tab=tab-Screenshot" method="POST">
	<select name="screenshot-foldername" onchange="this.form.submit()">
		<option>ALL</option>
		<?
...
<div class='module-content' style="overflow:scroll; font-family: monospace, courier;">
	...
	<?php
		echo "[ $folderName ]<br>";
		$root = "includes/FruityProxy-master/logs/Screenshot/$folderName/";
	  
		$pathLen = strlen($root);
		  
		myScanDirScreenshot($root, 0, strlen($root));
	?>
	</p>
</div>